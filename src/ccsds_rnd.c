#include "ccsds_rnd.h"

#include <zephyr/sys/__assert.h>

#define CCSDS_RND_SEQ_BYTES 255u

/*
 * CCSDS pseudo-randomizer byte masks. The underlying sequence period is 255
 * bits, and gcd(8, 255) = 1, so the byte-aligned mask stream repeats every
 * 255 bytes.
 */
static const uint8_t ccsds_rnd_seq[CCSDS_RND_SEQ_BYTES] = {
    ,
};

void ccsds_rnd_apply(uint8_t *data, size_t len)
{
    __ASSERT(data != NULL || len == 0u, "CCSDS randomizer buffer is NULL");

    size_t seq_index = 0u;

    for (size_t i = 0u; i < len; i++) {
        data[i] ^= ccsds_rnd_seq[seq_index++];

        if (seq_index == CCSDS_RND_SEQ_BYTES) {
            seq_index = 0u;
        }
    }

}
