...
/* GF(2^8) log table for CCSDS primitive polynomial 0x187. */
static const uint8_t GF_LOG[256] = {
    0x00, 0x00, 0x01, 0x63, 0x02, 0xc6, 0x64, 0x6a, 0x03, 0xcd, 0xc7, 0xbc, 0x65, 0x7e, 0x6b, 0x2a,
...
/* GF(2^8) inverse-log table for CCSDS primitive polynomial 0x187. */
static const uint8_t GF_ILOG[255] = {
...
/* Log forms of the standard CCSDS RS(255,223) generator coefficients. */
static const uint8_t GG_LOG[32] = {
    0x00, 0x66, 0xc8, 0x20, 0xcf, 0x8d, 0x7f, 0x65, 0x2b, 0x8e, 0x6a, 0x65, 0xac, 0x3c, 0x61, 0x4f,
                                  ...
                                  uint8_t parity[CCSDS_RS_PARITY_LEN])
{
    ...
    for (size_t i = 0u; i < CCSDS_RS_DATA_LEN; i++) {
        ...
        if (feedback != 0) {
            uint8_t feedback_log = GF_LOG[feedback];

            for (size_t j = CCSDS_RS_PARITY_LAST_INDEX; j > 0u; j--) {
                uint16_t idx = feedback_log + GG_LOG[j];
