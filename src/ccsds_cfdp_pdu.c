...
static void write_be_uint(uint8_t *buf, uint8_t len, uint64_t value)
{
    for (uint8_t i = 0u; i < len; i++) {
        uint8_t shift = (uint8_t)((len - 1u - i) * 8u);

        buf[i] = (uint8_t)(value >> shift);
    }
}
                         ...
                         size_t cap, size_t *len)
{
    ...
    buf[1] = (uint8_t)(header->pdu_data_field_len >> 8);
    buf[2] = (uint8_t)header->pdu_data_field_len;
                         ...
                         ccsds_cfdp_pdu_header_t *header, size_t *consumed)
{
    ...
    header->pdu_data_field_len = ((uint16_t)buf[1] << 8) | buf[2];
    header->segmentation_control =
